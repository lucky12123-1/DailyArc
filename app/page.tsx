...
export default function Home() {
  ...
  return <main className="app-frame">
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
      ...
      <nav><p className="nav-label">COMMAND CENTER</p>{nav("dashboard", "Today", LayoutDashboard)}{nav("history", "History", History)}{nav("deepPractice", "Deep Practice", GraduationCap)}<p className="nav-label spaced">ACCOUNT</p>{nav("settings", "Users & OpenAI", Users)}<span className="storage-note"><Archive size={15} /> Saved in Supabase</span></nav>
      <div className="sidebar-footer">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div className="avatar">{initials(activeUser.name)}</div>
            <div><b>{activeUser.name}</b><span>Evidence over intention</span></div>
          </div>
          <button
            title="Log out"
    ...
    <section className="main-area"><header className="topbar"><button className="mobile-menu" onClick={() => setMenuOpen(true)} aria-label="Open navigation"><Menu size={21} /></button><div><p className="eyebrow">{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(now)}</p><h1>{view === "dashboard" ? "Your day, measured honestly." : view === "history" ? "Your actual history." : view === "deepPractice" ? "Focused learning, made measurable." : "Your private workspace settings."}</h1></div><div className="top-actions"><div className="clock-display" aria-label="Current local time"><Clock3 size={15} /><span>{now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</span></div><span className="private-pill"><ShieldCheck size={14} /> Cloud-Synced</span><div className="avatar small">{initials(activeUser.name)}</div></div></header>
  ...
  return (
    <main className="workspace-setup">
      <section className="workspace-card animate-in">
        ...
        <p>
          {mode === "login"
            ? "Enter your display name and password to access your tracker and history."
            : "Set up a separate place for your plans, history, and optional OpenAI API key."}
        </p>
        ...
        >
          ...
          <label>
            Password {mode === "register" && <small>(at least 12 characters)</small>}
            <input
              type="password"
              value={password}
          ...
          <div className="truth-callout">
            <KeyRound size={17} />
            <span>Your password protects your personal workspace and is securely hashed.</span>
          </div>
          <button
            className="primary-button full"
            type="submit"
            disabled={loading || !name.trim() || !password || (mode === "register" && password.length < 12)}
          >
            {loading ? "Authenticating..." : mode === "login" ? "Sign In" : "Create Workspace"}{" "}
            <ArrowRight size={17} />
          </button>
        </form>
          ...
          {mode === "login" ? (
            <span>
              ...
              <button
                ...
                type="button"
                onClick={() => setMode("register")}
                style={{ background: "none", border: "none", color: "#285c4d", fontWeight: "bold", cursor: "pointer" }}
              >
                Create one now
              </button>
            </span>
